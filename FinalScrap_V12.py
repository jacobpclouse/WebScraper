...
def getJobs(inputUrl, outputFileName):
    ...
    #=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
        ...
        # Appending attribute
        job_attribute.append(number.text)
    ...
    #=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
        ...
        # Appending attribute
        job_attribute.append(grade.text)
    ...
    #=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
        ...
        # Appending attribute
        job_attribute.append(dueDate.text)
    ...
    #=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
        ...
        # Appending attribute
        job_attribute.append(agency.text)
    ...
    #=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
        ...
        # Appending attribute -- again spotty
        #job_attribute.append(links.text)


    #=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
    # Updating Dictionary
    #=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
        job_dictionary.update({job_key:job_attribute})
    print(job_dictionary)
    # Writing out text file to see what is in dictionary -- spotty
    #writeOutToFile(job_dictionary + '\n',f"dictionary_{outputFileName}")
